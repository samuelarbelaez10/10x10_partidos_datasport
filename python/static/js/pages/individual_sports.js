...
Pages.IndividualSports = async function(container) {
  ...
  try {
    ...
    if (indSports.length === 0) {
      container.innerHTML = '<div class="card" style="text-align:center;padding:40px;"><p style="color:var(--muted);">No se encontraron deportes individuales en Supabase.</p></div>';
      return;
    }

    const IC = { 'Gimnasia':'🤸','Tenis':'🎾','Golf':'⛳','Ajedrez':'♟️','Padel':'🏸','Pádel':'🏸','Natacion':'🏊','Natación':'🏊','Hipica':'🏇','Hípica':'🏇' };
    const CL = { 'Gimnasia':'#ec4899','Tenis':'#84cc16','Golf':'#22c55e','Ajedrez':'var(--purple)','Padel':'#f97316','Pádel':'#f97316','Natacion':'#06b6d4','Natación':'#06b6d4','Hipica':'#d97706','Hípica':'#d97706' };
    ...
    function logo(name, url, sz) {
      ...
      if (url) {
        return '<img src="' + url + '" style="width:' + sz + 'px;height:' + sz + 'px;border-radius:50%;object-fit:contain;background:white;padding:2px;flex-shrink:0;" onerror="this.style.display=\'none\';this.nextElementSibling.style.display=\'flex\';">' +
               '<div style="width:' + sz + 'px;height:' + sz + 'px;border-radius:50%;background:linear-gradient(135deg,var(--accent),var(--accent));display:none;align-items:center;justify-content:center;font-size:' + Math.round(sz/3) + 'px;font-weight:900;color:white;flex-shrink:0;">' + ini + '</div>';
      }
      return '<div style="width:' + sz + 'px;height:' + sz + 'px;border-radius:50%;background:linear-gradient(135deg,var(--accent),var(--accent));display:flex;align-items:center;justify-content:center;font-size:' + Math.round(sz/3) + 'px;font-weight:900;color:white;flex-shrink:0;">' + ini + '</div>';
    }
    ...
    function render() {
      ...
      var html = '<div style="margin-bottom:24px;">' +
        '<h2 style="font-size:28px;font-weight:900;color:var(--purple);display:flex;align-items:center;gap:10px;">🎖️ Deportes Individuales</h2>' +
        '<p style="font-size:13px;color:var(--muted);margin-top:4px;">Asigna medallas de oro, plata y bronce por colegio</p>' +
      '</div>';
      ...
      indSports.forEach(function(s) {
        var icon = IC[s.name] || '🏅';
        var color = CL[s.name] || 'var(--accent)';
        ...
        html += '<div onclick="window._indSelectSport(\'' + s.id + '\')" style="' +
          ...
          (cnt > 0 ? '<span style="font-size:11px;color:var(--muted);">' + cnt + ' medalla' + (cnt !== 1 ? 's' : '') + '</span>' : '') +
        '</div>';
      });
      html += '</div>';
      ...
      // --- SPORT SELECTED ---
      if (selSport) {
        var sc = CL[selSport.name] || 'var(--accent)';
          ...
          sportMedals.forEach(function(m) {
            ...
            html += '<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 0;border-bottom:1px solid var(--border);">' +
              '<div style="display:flex;align-items:center;gap:10px;">' +
                ...
                (m.description ? '<span style="font-size:11px;color:var(--muted);margin-left:6px;">(' + m.description + ')</span>' : '') +
              '</div>' +
              (isEditor ? '<button onclick="window._indDeleteMedal(\'' + m.id + '\')" style="background:rgba(239,68,68,0.12);border:1px solid rgba(239,68,68,0.25);color:var(--accent2);padding:4px 12px;border-radius:8px;cursor:pointer;font-size:11px;font-family:inherit;">Eliminar</button>' : '') +
            '</div>';
          });
          html += '</div>';
        }
        ...
        html += '<h3 style="font-size:15px;font-weight:700;color:var(--muted);margin-bottom:14px;">Selecciona un colegio para asignar medalla</h3>';
        ...
        if (selSchool && isEditor) {
          ...
          html += '<div style="display:flex;align-items:center;gap:12px;margin-bottom:16px;">' +
            ...
            '<div>' +
              ...
              '<div style="font-size:12px;color:var(--muted);">Asignar medalla en ' + selSport.name + '</div>' +
            '</div>' +
          '</div>';
          html += '<div style="margin-bottom:14px;">' +
            '<label style="font-size:12px;color:var(--muted);font-weight:600;">Descripcion (opcional)</label>' +
          ...
          '</div>';
          html += '<div style="margin-bottom:14px;">' +
            '<label style="font-size:12px;color:var(--muted);font-weight:600;">Cantidad de medallas</label>' +
          ...
          html += '<div style="display:flex;gap:10px;">' +
            ...
            '<button onclick="window._indAddMedal(\'silver\')" style="flex:1;padding:14px;border-radius:14px;cursor:pointer;background:linear-gradient(135deg,#374151,#4b5563);border:2px solid var(--muted);color:white;font-weight:800;font-size:15px;font-family:inherit;box-shadow:0 4px 16px rgba(148,163,184,0.25);transition:all .2s;">🥈 Plata</button>' +
        ...
        html += '<div class="card" style="text-align:center;padding:48px;">' +
          ...
          '<p style="font-size:16px;color:var(--muted);">Selecciona un deporte para ver y asignar medallas</p>' +
        '</div>';
      }
